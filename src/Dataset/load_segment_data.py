...
def load(config):
    """
    Load and preprocess multivariate time series data for deep learning classification.

    This function handles three types of datasets: ActivityRecognition, FordChallenge, and 
    Emotiv-based segmentation data. It checks for the existence of a preprocessed `.npy` file
    and loads it if available. Otherwise, it reads raw CSV or TXT data, processes it using 
    dataset-specific loaders, segments the data into subsequences using a sliding window, 
    and splits it into training, validation, and test sets.

    The final output is a dictionary with shape-transposed time series data and corresponding labels.
    All arrays are shaped as (samples, channels, timesteps), suitable for Conv/Transformer-based models.

    Parameters:
        config (dict): Dictionary containing configuration options.
                       Must include key 'data_dir' pointing to the dataset directory.

    Returns:
        dict: A dictionary with the following keys:
              - 'train_data': np.ndarray of shape (n_train, n_channels, window_len)
              - 'train_label': np.ndarray of training labels
              - 'val_data': np.ndarray of shape (n_val, n_channels, window_len)
              - 'val_label': np.ndarray of validation labels
              - 'test_data': np.ndarray of shape (n_test, n_channels, window_len)
              - 'test_label': np.ndarray of test labels
              - 'max_len': int, window length used for subsequences
    """
    ...
    if os.path.exists(config['data_dir'] + '/' + problem + '.npy'):
        ...
        if problem == "ActivityRecognition":
            ...
            train_data, test_data = data_loader.load_activity(
                all_file, "Clean")
        ...
        # Emotiv Datasets
            ...
            train_file = config['data_dir'] + '/' + problem + "_TRAIN.csv"
            test_file = config['data_dir'] + '/' + problem + "_TEST.csv"
            train_data = data_loader.load_segmentation_data(
                train_file, "Clean")
        ...
        Data['train_data'] = X_train.reshape(
            X_train.shape[0], X_train.shape[2], X_train.shape[1])
        Data['train_label'] = y_train

        Data['val_data'] = X_val.reshape(
            X_val.shape[0], X_val.shape[2], X_val.shape[1])
        Data['val_label'] = y_val

        Data['test_data'] = X_test.reshape(
